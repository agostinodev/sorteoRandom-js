import { mostrarAlertas } from './index.js';
 ...
 export const validarLista = (lista) => {
    ...
    if(lista.length === 0) return mostrarAlertas("informacion",'No ingreso ningún nombre.');
    ...
    if(lista.some(el => /\d/.test(el))){
        mostrarAlertas("informacion",'El nombre NO debe tener números.');
        return false;
    }
    ...
    if(lista.some(el => !/^[a-zA-Záäéëíïóöúü\s]+$/.test(el))){
        mostrarAlertas("informacion",'El nombre NO debe tener caracteres no alfanuméricos.');
        return false;
    };

    //Valida nombres repetidos.
    if(lista.some( (el, i, array) => array.indexOf(el) != i)){
        mostrarAlertas("informacion", 'No puede haber nombres repetidos.');
        return false;
    };
